...
*   **Flow:**
    ...
    4.  Парсинг выполняется в фоне (BackgroundTasks), секции автоматически сохраняются в таблицу `source_sections`.
...
### Структура таблицы `source_sections`
*   **Основные поля:**
...
*   Для каждой секции в `source_sections` генерируется эмбеддинг.
